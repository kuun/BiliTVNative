...
| P4-33 | Dolby/Hi-Res 音频请求与默认音频设置 | Done | 播放 URL 请求追加 Dolby Audio `fnval`，解析 `dash.audio`、`dash.dolby.audio` 和 `dash.flac.audio`；新增默认音频设置，包含最高、Hi-Res/FLAC、Dolby Atmos、普通最高，默认最高按 Hi-Res/FLAC > Dolby > 普通 192K/132K/64K 回退，普通最高会避开高规格音轨；`assembleDebug` 通过，已安装到 `192.168.2.20:5555` |
| P4-34 | 播放器音频格式显示与快捷切换 | Done | 播放器底部画质按钮右侧新增音频格式按钮，显示当前 Hi-Res/FLAC、Dolby Atmos 或普通 192K/132K/64K；播放器设置主菜单新增“音频格式”行，右侧音频面板列出本次 playurl 返回的可选音轨并标记当前项；选择音轨后保持当前位置并重建当前 DASH media source，显式选择时只暴露所选音轨，避免 ExoPlayer 自适应回跳；`assembleDebug` 通过，已安装到 `192.168.2.20:5555` |
...
| P9-84 | 播放器控制栏直接选择画质 | Done | TV 播放器底部右侧原画质状态位置改为可聚焦的画质文字按钮，只使用 `4K/1080P60/1080P+/1080P/720P` 等短标签显示当前画质，短标签优先从菜单里的实际画质描述解析再用 qn id 兜底，避免菜单没有 `1080P+` 时控制栏误显示；未聚焦时画质按钮不显示边框/玻璃背景，聚焦时再显示焦点视觉；选中后按 OK 直接弹出画质选择列表，列表聚焦当前画质并保留当前标记，选择后从当前位置续播，不再需要先进入播放设置菜单；右侧状态区保留在线人数和弹幕数；`git diff --check` 和 `ANDROID_HOME=/Users/kevin/Library/Android/sdk bash gradlew :app:assembleDebug -Dorg.gradle.java.home=/Library/Java/JavaVirtualMachines/zulu-17.jdk/Contents/Home` 通过 |
